...
\\
\textbf{UniFrac:} Developed in 2005\cite{lozupone2005unifrac}, UniFrac is a commonly used distance metric in microbiome studies. It measures the distance between samples and enables subsequent multivariate analyses such as principal coordinate analysis.\\
\\
...
\Large\begin{center}\textbf{Statement of work}\end{center}\normalsize

Jia R. Wu performed the sample migration experiment and generated the relevant figures for the \textit{Expanding the UniFrac toolbox} chapter. He also wrote the original draft of the relevant methods sections, however these were edited and incorporated into the chapter by myself. The chapter has since been submitted to PLOS ONE as a standalone paper.

The sample collection and DNA extraction for the non alcoholic fatty liver disease experiments were done by the Allard lab research group, as described in \textit{Appendix B}. I ran a PCR to amplify the DNA for the 16S rRNA gene tag sequencing experiment. The actual sequencing for both the gene tag and the metageonomic experiments were performed by the London Regional Genomics Centre. Dr. Greg Gloor ran the steps leading up to the generation of the table of counts per taxa per sample for the 16S rRNA gene tag experiment. I completed the downstream analysis for the 16S rRNA gene tag experiment, as well as all of the data processing and analysis post-sequencing for the metagenomic experiment.
...
\include{unifrac_paper/expanding_the_unifrac_toolbox_plain_formatting}
\include{nafld}
\include{discussion}
